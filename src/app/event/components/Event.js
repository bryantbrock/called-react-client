    ...
    return event ? (
      <div className="events-root mb-5">
        ...
        <Row>
          <Col xs={12} md={6} className="bg-dark p-5 text-white">
            <div className="container text-center">
              ...
              <h1 className="mt-4 display-4"><Countdown date={event.start_date} /></h1>
        ...
        <div className="container mt-5">
          ...
          <ListGroup>
            {registrant_items.map((registrant, index) => <LinkContainer to={`/event/${event.pk}/registrant/${registrant.pk}`}><ListGroup.Item action href="#" key={index}>{registrant.name}{(registrant.paid == true && registrant.number_of_unanswered_additional_forms > 0 ? <Badge className="ml-2 float-right" variant="info">{registrant.number_of_unanswered_additional_forms} Unfilled Form{(registrant.number_of_unanswered_additional_forms != 1 ? 's' : '')}</Badge> : '')}{(registrant.paid == false ? <Badge className="ml-2 float-right" variant="info">Registration incomplete</Badge> : '')}</ListGroup.Item></LinkContainer>)}
